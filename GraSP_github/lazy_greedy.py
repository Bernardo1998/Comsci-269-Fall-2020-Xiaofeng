...
class FacilityLocation:
    ...
    def __init__(self, D, V, alpha=1.):
        ...
        self.norm = 1. / self.inc(V, []) #Initial norm would be log2

    def f_norm(self, sset):
        return self.D[:, sset].max(axis=1).sum() #Sum of maximum in each row

    def inc(self, sset, ndx):
        if len(sset + [ndx]) > 1:#If subset is not empty
            if not ndx:  # normalization, if ndx is empty
    ...
    def add(self, sset, ndx):
        ...
        self.gains.extend([self.curVal - cur_old])#Check marginal
        return self.curVal
...
def test():
    ...
    D = X * np.transpose(X)
    F = FacilityLocation(D, list(range(0, n)))
