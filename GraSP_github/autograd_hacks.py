...
_enforce_fresh_backprop: bool = True   # global switch to catch double backprop errors on Hessian computation
...
def compute_grad1(model: nn.Module, loss_type: str = 'mean') -> None:
    ...
    for k,layer in enumerate(model.modules()):
        ...
        if layer_type == 'Linear': #Directly apply the formula on BN layers.
            #print(f"{layer_type} weights: ",layer.weight.data.size())
            setattr(layer.weight, 'grad1', torch.einsum('ni,nj->nij', B, A))
            if layer.bias is not None:
                setattr(layer.bias, 'grad1', B)

        elif layer_type == 'Conv2d':
            #print(f'{layer_type} layer, show size/AB shapes')
            ...
            stride=layer.stride
            A = torch.nn.functional.unfold(A, kernel_size=kernel_size, dilation=dilation, padding=padding, stride=stride)
            #print(B.shape,A.shape)

            B = B.reshape(n, -1, A.shape[-1])

            #B = B.reshape(n, n_weights, -1) # I changed how it takes the weights
           # print(B.shape, A.shape)
            grad1 = torch.einsum('ijk,ilk->ijl', B, A)
                
            #print(grad1.shape)
            shape = [n] + list(layer.weight.shape)
            #print(grad1.shape,shape)
            ...
            #continue
            #print("BN layer:",B.shape,A.shape)
            #print(layer.weight.data.size())
            #A = torch.nn.functional.unfold(A, kernel_size=kernel_size, dilation=dilation, padding=padding, stride=stride)
            A = torch.nn.functional.unfold(A, kernel_size=kernel_size,padding=padding)
            #B = B.reshape(n, -1, A.shape[-1])

            B = B.reshape(n, n_weights, -1) # I changed how it takes the weights
            #print(B.shape, A.shape)
            grad1 = torch.einsum('ijk,ilm->ij', B, A)
            #grad1 = torch.einsum('ijkm,ijkm->ij', B, A)
